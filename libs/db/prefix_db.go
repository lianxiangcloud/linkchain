...
func (pdb *prefixDB) Load(key []byte) ([]byte, error) {
	pdb.mtx.Lock()
	defer pdb.mtx.Unlock()

	pkey := pdb.prefixed(key)
	return pdb.db.Load(pkey)
}
...
func (pdb *prefixDB) Exist(key []byte) (bool, error) {
	pdb.mtx.Lock()
	defer pdb.mtx.Unlock()

	return pdb.db.Exist(pdb.prefixed(key))
}
...
func (pdb *prefixDB) Put(key []byte, value []byte) error {
	pdb.mtx.Lock()
	defer pdb.mtx.Unlock()

	pkey := pdb.prefixed(key)
	return pdb.db.Put(pkey, value)
}
...
func (pdb *prefixDB) Del(key []byte) error {
	pdb.mtx.Lock()
	defer pdb.mtx.Unlock()

	return pdb.db.Del(pdb.prefixed(key))
}
...
func (pdb *prefixDB) NewIteratorWithPrefix(prefix []byte) Iterator {
	start := prefix
...
func (pb prefixBatch) ValueSize() int {
	return pb.source.ValueSize()
}
...
func newPrefixIterator(prefix, start, end []byte, source Iterator, isReverse bool, db DB) prefixIterator {
...
func (itr prefixIterator) Next() (valid bool) {
...
func (itr prefixIterator) Seek(key []byte) bool {
	if itr.isReverse {
		var pend []byte
		if key == nil {
			// This may cause the underlying iterator to end with an
			...
			// prefixIterator will terminate iteration
			// automatically upon detecting this.
			pend = cpDecr(itr.prefix)
		} else {
			pend = append(cp(itr.prefix), key...)
		}
		ritr := itr.db.ReverseIterator(itr.start, pend)
		itr.source = ritr
	} else {
		pstart := append(cp(itr.prefix), key...)
		tmpItr := itr.db.Iterator(
			pstart,
			itr.end,
		)
		itr.source = tmpItr
