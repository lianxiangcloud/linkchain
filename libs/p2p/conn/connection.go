...
const (
	defaultMaxPacketMsgPayloadSize = 32 * 1024
	//defaultMaxPacketMsgPayloadSize = 5 * 1024 * 1024
	numBatchPacketMsgs = 1000
	minReadBufferSize  = 32 * 1024
	...
	defaultFlushThrottle = 73 * time.Millisecond

	defaultSendQueueCapacity   = 100
	defaultRecvBufferCapacity  = 128 * 1024     // 128k
...
func (c *MConnection) stopForError(r interface{}) {
	...
	if atomic.CompareAndSwapUint32(&c.errored, 0, 1) {
		if c.onError != nil {
			c.onError(r) //call sw.StopPeerForError
		}
	}
}
...
FOR_LOOP:
	...
	SELECTION:
		...
		case <-c.pingTimer.Chan():
			...
			_n, err = ser.EncodeWriterWithType(c.bufConnWriter, PacketPing{})
			if err != nil {
				break SELECTION
			}
		...
		case <-c.pong:
			...
			_n, err = ser.EncodeWriterWithType(c.bufConnWriter, PacketPong{})
			if err != nil {
				break SELECTION
			}
			c.sendMonitor.Update(int(_n))
...
FOR_LOOP:
	for {
		...
		var _n int64
		var err error
		_n, err = ser.DecodeReaderWithType(c.bufConnReader, &packet, int64(c._maxPacketMsgSize))
...
func (c *MConnection) maxPacketMsgSize() int {
	return len(ser.MustEncodeToBytesWithType(PacketMsg{
		ChannelID: 0x01,
		EOF:       1,
...
func (ch *Channel) writePacketMsgTo(w io.Writer) (n int64, err error) {
	...
	n, err = ser.EncodeWriterWithType(w, packet)
	ch.recentlySent += n
	return
}
...
func RegisterPacket() {
	...
	ser.RegisterConcrete(PacketPing{}, "p2p/PacketPing", nil)
	ser.RegisterConcrete(PacketPong{}, "p2p/PacketPong", nil)
	ser.RegisterConcrete(PacketMsg{}, "p2p/PacketMsg", nil)
}
