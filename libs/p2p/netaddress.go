...
// Errors are of type ErrNetAddressXxx where Xxx is in (Invalid, Lookup)
...
func GetLocalAllAddress() []string {
	...
	for _, a := range addrs {
		...
		if na.RFC1918() {
			privateAddrs = append(privateAddrs, ipnet.IP.String())
		}
		if !na.Routable() {
			continue
		}
	...
	if len(localaddr) == 0 {
		localaddr = append(localaddr, privateAddrs...)
	}
	return localaddr
}
