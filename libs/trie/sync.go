...
func NewSync(root common.Hash, database DatabaseReader, callback LeafCallback) *Sync {
	...
	ts.AddSubTrie(root, 0, common.EmptyHash, callback)
	return ts
}
...
func (s *Sync) AddRawEntry(hash common.Hash, depth int, parent common.Hash) {
	...
	if ok, _ := s.database.Exist(hash.Bytes()); ok {
		return
	}
	// Assemble the new sub-trie sync request
	req := &request{
...
func (s *Sync) Commit(dbw dbm.Putter) (int, error) {
...
func (s *Sync) children(req *request, object node) ([]*request, error) {
	...
	for _, child := range children {
		...
		if node, ok := (child.node).(hashNode); ok {
			...
			if ok, _ := s.database.Exist(node); ok {
				continue
			}
