...
// Used for testing
func newEmpty() *Trie {
	trie, _ := New(common.EmptyHash, NewDatabase(dbm.NewMemDB()))
	return trie
}
...
func TestMissingRoot(t *testing.T) {
	trie, err := New(common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"), NewDatabase(dbm.NewMemDB()))
	if trie != nil {
...
func testMissingNode(t *testing.T, memonly bool) {
	...
	trie, _ := New(common.EmptyHash, triedb)
...
func (db *countingDB) Load(key []byte) ([]byte, error) {
	db.gets[string(key)]++
	return db.DB.Load(key)
}
...
func runRandTest(rt randTest) bool {
	triedb := NewDatabase(dbm.NewMemDB())

	tr, _ := New(common.EmptyHash, triedb)
	...
	for i, step := range rt {
		...
		case opItercheckhash:
			checktr, _ := New(common.EmptyHash, triedb)
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		_, tmpdb := tempDB()
		trie, _ = New(common.EmptyHash, tmpdb)
	}
	k := make([]byte, 32)
	...
	if commit {
		ldb := trie.db.diskdb.(*dbm.GoLevelDB)
		ldb.Close()
		os.RemoveAll(ldb.Dir())
	}
}
...
func BenchmarkHash(b *testing.B) {
	...
	for i := 0; i < len(accounts); i++ {
		...
		accounts[i], _ = ser.EncodeToBytes([]interface{}{nonce, balance, root, code})
	}
...
func tempDB() (string, *Database) {
	...
	diskdb, err := dbm.NewGoLevelDB("", dir, 1)
	if err != nil {
