...
type DatabaseReader interface {
	// Load retrieves the value associated with key form the database.
	Load(key []byte) (value []byte, err error)

	// Exist retrieves whether a key is present in the database.
...
type Database struct {
	diskdb dbm.DB // Persistent storage for matured trie nodes
...
// nodes and already encoded SER binary blobs (while at the same time store them
...
// the same SER encoding as the original parent.
...
func (n rawFullNode) EncodeSER(w io.Writer) error {
	var nodes [17]node
...
type cachedNode struct {
	node node   // Cached collapsed trie node, or raw ser data
...
func (n *cachedNode) String() string {
	if n == nil {
		return "nil"
	}
	return fmt.Sprintf("{prev:%s, next:%s}", n.flushPrev.String(), n.flushNext.String())
}

// ser returns the raw ser encoded blob of the cached node, either directly from
// the cache, or by regenerating it from the collapsed node.
func (n *cachedNode) ser() []byte {
	if node, ok := n.node.(rawNode); ok {
		return node
	}
	blob, err := ser.EncodeToBytes(n.node)
	if err != nil {
		panic(err)
	}
	return blob
...
// or by regenerating it from the ser encoded blob.
...
func NewDatabase(diskdb dbm.DB) *Database {
	return &Database{
		diskdb:    diskdb,
...
func (db *Database) String() string {
	...
	return fmt.Sprintf("{nodes:%d, oldest_hash:%s, newest_hash:%s, oldest:%s, newest:%s}", len(db.nodes), db.oldest.String(), db.newest.String(),
		db.nodes[db.newest].String(), db.nodes[db.oldest].String())
}
...
func (db *Database) preimage(hash common.Hash) ([]byte, error) {
	...
	// Content unavailable in memory, attempt to retrieve from disk
	return db.diskdb.Load(db.secureKey(hash[:]))
}
...
func (db *Database) Nodes() []common.Hash {
	...
	for hash := range db.nodes {
		if hash != (common.EmptyHash) { // Special case for "root" references/nodes
...
func (db *Database) reference(child common.Hash, parent common.Hash) {
	...
	} else if _, ok = db.nodes[parent].children[child]; ok && parent != (common.EmptyHash) {
		return
	}
	node.parents++
...
func (db *Database) Dereference(root common.Hash) {
	...
	db.dereference(root, common.EmptyHash)
...
func (db *Database) dereference(child common.Hash, parent common.Hash) {
	...
	if node.parents == 0 {
		...
		if child == db.oldest {
			...
			db.nodes[node.flushNext].flushPrev = common.EmptyHash
		} else if child == db.newest {
			db.newest = node.flushPrev
			db.nodes[node.flushPrev].flushNext = common.EmptyHash
		} else {
...
func (db *Database) verifyIntegrity() {
	...
	for child := range db.nodes[common.EmptyHash].children {
