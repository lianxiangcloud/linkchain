...
func TestEmptySync(t *testing.T) {
	dbA := NewDatabase(dbm.NewMemDB())
	dbB := NewDatabase(dbm.NewMemDB())
	emptyA, _ := New(common.EmptyHash, dbA)
	emptyB, _ := New(emptyRoot, dbB)

	for i, trie := range []*Trie{emptyA, emptyB} {
		if req := NewSync(trie.Hash(), dbm.NewMemDB(), nil).Missing(1); len(req) != 0 {
