...
func (cdc *Codec) getRegistedTypeInfo_wlock(rt reflect.Type) (info *TypeInfo, registed bool) {
...
func addDisfix(val reflect.Value, w *encbuf) (err error) {
	var rt = val.Type()
	var cinfo *TypeInfo
	cinfo, registed := cdc.getRegistedTypeInfo_wlock(rt)
...
func writeCDCInterface(rv reflect.Value, w *encbuf) (err error) {
	...
	_, isRegisted := cdc.getRegistedTypeInfo_wlock(drt)
	if !isRegisted {
	...
	var cinfo *TypeInfo
	cinfo, isRegisted = cdc.getRegistedTypeInfo_wlock(crt)
...
func decodeCDCInterface(s *Stream, rv reflect.Value) (err error) {
	...
	copy(disfixbyte[:], bz[:])
	cinfo, err = cdc.getTypeInfoFromDisfix_rlock(disfixbyte)
	if err != nil {
		return
	}
...
//EncodeByteSlice wrapper for cdc.EncodeByteSlice, but use rlp encode
func EncodeByteSlice(w io.Writer, bz []byte) (err error) {
	_, err = EncodeWriter(w, bz)
	return err
}

//EncodeUvarint wrapper for cdc.EncodeUvarint, but use rlp encode
