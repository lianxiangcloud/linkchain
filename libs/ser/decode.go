...
//
// The DecodeSER method should read one value from the given
// Stream. It is not forbidden to read less or more, but it might
// be confusing.
type Decoder interface {
	DecodeSER(*Stream) error
}
...
func makeMapDecoder(typ reflect.Type) (decoder, error) {
	...
	default:
		return nil, fmt.Errorf("rlp: type %v is not RLP-decode", typ)
	}
	switch {
	...
	dec = func(s *Stream, val reflect.Value) (err error) {
		if !val.CanAddr() {
			return fmt.Errorf("rlp: val %v is not addressable", val)
		}
...
func decodeDecoderNoPtr(s *Stream, val reflect.Value) error {
	return val.Addr().Interface().(Decoder).DecodeSER(s)
}
...
func decodeDecoder(s *Stream, val reflect.Value) error {
	...
	return val.Interface().(Decoder).DecodeSER(s)
}
