...
var (
	...
	// These are useful when implementing EncodeSER.
...
type Encoder interface {
	// EncodeSER should write the RLP encoding of its receiver to w.
...
// EncodeSER. This is true even for nil pointers, please see the
...
func Encode(w io.Writer, val interface{}) error {
	if outer, ok := w.(*encbuf); ok {
		// Encode was called by some type's EncodeSER.
...
//EncodeWithType return RLP encode with 7 bytes prefix when the type of val is registed
...
// encbuf implements io.Writer so it can be passed it into EncodeSER.
...
func writeEncoder(val reflect.Value, w *encbuf) error {
	return val.Interface().(Encoder).EncodeSER(w)
}
...
func writeEncoderNoPtr(val reflect.Value, w *encbuf) error {
	if !val.CanAddr() {
		...
		return fmt.Errorf("rlp: game over: unadressable value of type %v, EncodeSER is pointer method", val.Type())
	}
	return val.Addr().Interface().(Encoder).EncodeSER(w)
}
