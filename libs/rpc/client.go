...
func (c *Client) CallContext(ctx context.Context, result interface{}, method string, args ...interface{}) error {
	...
	case len(resp.Result) == 0:
		return ErrNoResult
	default:
		if isHexNum(reflect.TypeOf(result)) {
			//val := reflect.New(reflect.TypeOf((**hexutil.Big)(nil)).Elem())
			var val *hexutil.Big
			if err := json.Unmarshal(resp.Result, &val); err != nil {
				return fmt.Errorf("failed to decode hex num: %v", err)
			}
			reflect.ValueOf(result).Elem().Set(reflect.ValueOf(val.ToInt()))
			return nil
		}
