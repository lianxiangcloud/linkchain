...
func makeVoteHR(t *testing.T, height uint64, round int, privVals []types.PrivValidator, valIndex int) *types.Vote {
	privVal := privVals[valIndex]
	vote := &types.Vote{
		ValidatorAddress: privVal.GetAddress(),
		ValidatorIndex:   valIndex,
		ValidatorSize:    len(privVals),
		Height:           height,
		Round:            round,
		Timestamp:        time.Now().UTC(),
		Type:             types.VoteTypePrecommit,
		BlockID:          types.BlockID{common.BytesToHash([]byte("fakehash")), types.PartSetHeader{}},
	}
