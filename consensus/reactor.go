...
func (conR *ConsensusReactor) SwitchToConsensus(status NewStatus, blocksSynced int) {
	...
	conR.switchMtx.Lock()
	defer conR.switchMtx.Unlock()

	conR.mtx.Lock()
	if !conR.fastSync {
		conR.mtx.Unlock()
		conR.Logger.Warn("Already in Consensus mode")
		return
	}
	conR.fastSync = false
	conR.mtx.Unlock()
	conR.conS.SetReceiveP2pTx(conR.receiveP2pTx)

	conR.conS.CommitRound = -1
	// NOTE: The line below causes broadcastNewRoundStepRoutine() to
	// broadcast a NewRoundStepMessage.
	conR.conS.updateToStatus(status)
	conR.conS.reconstructLastCommit(status)
...
type blockchainReactor interface {
	...
	RestartFastSync(status NewStatus) error
}

// SwitchToFastSync switches from consensus mode to fast_sync mode.
...
func (conR *ConsensusReactor) SwitchToFastSync() {
	...
	if conR.fastSync {
		conR.mtx.Unlock()
		conR.Logger.Warn("Already in FastSync mode")
		return
	}
	conR.fastSync = true
	conR.mtx.Unlock()
	...
	err := conR.conS.Stop()
	if err != nil {
		conR.Logger.Error("Error stop conS", "err", err)
	}
	...
	err = conR.conS.Reset()
	if err != nil {
		conR.Logger.Error("Error reset conS", "err", err)
	}

	bcR := conR.sw.Reactor("BLOCKCHAIN").(blockchainReactor)
	err = bcR.RestartFastSync(conR.conS.GetState())
	if err != nil {
		conR.Logger.Error("Error Restart bcR", "err", err)
	}
}

func (conR *ConsensusReactor) StopTheWorld() bool {
	conR.Logger.Info("StopTheWorld")

	conR.switchMtx.Lock()
	defer conR.switchMtx.Unlock()

	if !conR.started {
		conR.Logger.Warn("StopTheWorld: Already stopped")
		return false
	}
	conR.started = false
	...
	bcR := conR.sw.Reactor("BLOCKCHAIN").(blockchainReactor)
	if bcR.StopFastSync() {
		conR.Logger.Info("StopTheWorld: StopFastSync in FastSync mode")
		conR.switchMtx.Unlock()
		//release lock ,wait to consensus
		for conR.FastSync() {
			time.Sleep(73 * time.Millisecond)
		}
		conR.Logger.Info("StopTheWorld: back to consensus when StopFastSync in FastSync mode")
		conR.switchMtx.Lock()
	}
	...
	conR.fastSync = true
	conR.mtx.Unlock()
	conR.conS.SetReceiveP2pTx(false)

	err := conR.conS.Stop()
	if err != nil {
		conR.Logger.Error("StopTheWorld: Error stop conS", "err", err)
	}
	...
	err = conR.conS.Reset()
	if err != nil {
		conR.Logger.Error("StopTheWorld: Error reset conS", "err", err)
	}
	return true
}

func (conR *ConsensusReactor) StartTheWorld() bool {
	conR.Logger.Info("StartTheWorld")

	conR.switchMtx.Lock()
	defer conR.switchMtx.Unlock()

	if conR.started {
		conR.Logger.Warn("StartTheWorld: Already started")
		return false
	}
	conR.started = true

	if conR.conS.Validators.Size() == 1 {
		// one node mode
		conR.mtx.Lock()
		conR.fastSync = false
		conR.mtx.Unlock()
		...
		err := conR.conS.Start()
		if err != nil {
			conR.Logger.Error("StartTheWorld: Error starting conS", "err", err)
		}
		return true
	}
	...
	err := bcR.RestartFastSync(conR.conS.GetState())
	if err != nil {
		conR.Logger.Error("StartTheWorld: Error Restart bcR", "err", err)
	}
	return true
}
...
func (conR *ConsensusReactor) GetChannels() []*p2p.ChannelDescriptor {
	...
	return []*p2p.ChannelDescriptor{
		...
		},
		{
			...
			Priority:            100,         // once we gossip the whole block there's nothing left to send until next height or round
			SendQueueCapacity:   100,
...
func (conR *ConsensusReactor) Receive(chID byte, src p2p.Peer, msgBytes []byte) {
	...
	if err != nil {
		...
		conR.sw.StopPeerForError(src, err)
		return
	}
	...
	var ps *PeerState = nil
	for {
		psk := src.Get(types.PeerStateKey)
		if psk != nil {
			ps = psk.(*PeerState)
			break
		}
		conR.Logger.Info("Wait Receive msg", "src", src, "chId", chID, "msg", msg)
	...
	case StateChannel:
		...
		case *VoteSetMaj23Message:
			...
			if err != nil {
				conR.sw.StopPeerForError(src, fmt.Errorf("SetPeerMaj23: %v", err))
				return
			}
			...
			src.TrySend(VoteSetBitsChannel, ser.MustEncodeToBytesWithType(&VoteSetBitsMessage{
				Height:  msg.Height,
				Round:   msg.Round,
				Type:    msg.Type,
				BlockID: msg.BlockID,
...
func (conR *ConsensusReactor) broadcastProposalHeartbeatMessage(hb *types.Heartbeat) {
	...
	conR.sw.Broadcast(StateChannel, ser.MustEncodeToBytesWithType(msg))
}

func (conR *ConsensusReactor) broadcastNewRoundStepMessages(rs *cstypes.RoundState) {
	nrsMsg, csMsg := makeRoundStepMessages(rs)
	if nrsMsg != nil {
		conR.sw.Broadcast(StateChannel, ser.MustEncodeToBytesWithType(nrsMsg))
	}
	if csMsg != nil {
		conR.sw.Broadcast(StateChannel, ser.MustEncodeToBytesWithType(csMsg))
	}
}
...
func (conR *ConsensusReactor) sendNewRoundStepMessages(peer p2p.Peer) {
	...
	nrsMsg, csMsg := makeRoundStepMessages(rs)
	if nrsMsg != nil {
		peer.Send(StateChannel, ser.MustEncodeToBytesWithType(nrsMsg))
	}
	if csMsg != nil {
		peer.Send(StateChannel, ser.MustEncodeToBytesWithType(csMsg))
	}
}
	...
	for {
		...
		if rs.ProposalBlockParts.HasHeader(prs.ProposalBlockPartsHeader) {
			if index, ok := rs.ProposalBlockParts.BitArray().Sub(prs.ProposalBlockParts.Copy()).PickRandom(); ok {
				...
				if peer.Send(DataChannel, ser.MustEncodeToBytesWithType(msg)) {
		...
		if (0 < prs.Height) && (prs.Height < rs.Height) {
			...
			if prs.ProposalBlockParts == nil {
				blockMeta := conR.conS.appmgr.LoadBlockMeta(prs.Height)
				if blockMeta == nil {
					cmn.PanicCrisis(cmn.Fmt("Failed to load block %d when blockStore is at %d",
						prs.Height, conR.conS.appmgr.Height()))
				}
		...
		if rs.Proposal != nil && !prs.Proposal {
			...
			if 0 <= rs.Proposal.POLRound {
				...
				peer.Send(DataChannel, ser.MustEncodeToBytesWithType(msg))
			}
			continue OUTER_LOOP
		}
...
func (conR *ConsensusReactor) gossipDataForCatchup(logger log.Logger, rs *cstypes.RoundState,
	...
	peerLogger := logger.With("peer", peer)
	if index, ok := prs.ProposalBlockParts.Not().PickRandom(); ok {
		// Ensure that the peer's PartSetHeader is correct
		blockMeta := conR.conS.appmgr.LoadBlockMeta(prs.Height)
		if blockMeta == nil {
			peerLogger.Error("Failed to load block meta",
				"ourHeight", rs.Height, "blockstoreHeight", conR.conS.appmgr.Height())
			time.Sleep(conR.conS.config.PeerGossipSleep())
			return
		} else if !blockMeta.BlockID.PartsHeader.Equals(prs.ProposalBlockPartsHeader) {
			peerLogger.Info("Peer ProposalBlockPartsHeader mismatch, sleeping",
				"blockPartsHeader", blockMeta.BlockID.PartsHeader, "peerBlockPartsHeader", prs.ProposalBlockPartsHeader)
			time.Sleep(conR.conS.config.PeerGossipSleep())
			return
		}
		// Load the part
		part := conR.conS.appmgr.LoadBlockPart(prs.Height, index)
		if part == nil {
			peerLogger.Error("Could not load part", "index", index,
				"blockPartsHeader", blockMeta.BlockID.PartsHeader, "peerBlockPartsHeader", prs.ProposalBlockPartsHeader)
			time.Sleep(conR.conS.config.PeerGossipSleep())
			return
		}
		...
		peerLogger.Debug("Sending block part for catchup", "round", prs.Round, "index", index)
		if peer.Send(DataChannel, ser.MustEncodeToBytesWithType(msg)) {
			ps.SetHasProposalBlockPart(prs.Height, prs.Round, index)
		} else {
			peerLogger.Debug("Sending block part for catchup failed")
		}
		return
	}
	//peerLogger.Info("No parts to send in catch-up, sleeping")
	time.Sleep(conR.conS.config.PeerGossipSleep())
}
...
func (ps *PeerState) PickSendVote(votes types.VoteSetReader) bool {
	if vote, ok := ps.PickVoteToSend(votes); ok {
		...
		return ps.peer.Send(VoteChannel, ser.MustEncodeToBytesWithType(msg))
	}
	return false
}
...
func RegisterConsensusMessages() {
	ser.RegisterInterface((*ConsensusMessage)(nil), nil)
	ser.RegisterConcrete(&NewRoundStepMessage{}, "consensus/NewRoundStepMessage", nil)
	ser.RegisterConcrete(&CommitStepMessage{}, "consensus/CommitStep", nil)
	ser.RegisterConcrete(&ProposalMessage{}, "consensus/Proposal", nil)
	ser.RegisterConcrete(&ProposalPOLMessage{}, "consensus/ProposalPOL", nil)
	ser.RegisterConcrete(&BlockPartMessage{}, "consensus/BlockPart", nil)
	ser.RegisterConcrete(&VoteMessage{}, "consensus/Vote", nil)
	ser.RegisterConcrete(&HasVoteMessage{}, "consensus/HasVote", nil)
	ser.RegisterConcrete(&VoteSetMaj23Message{}, "consensus/VoteSetMaj23", nil)
	ser.RegisterConcrete(&VoteSetBitsMessage{}, "consensus/VoteSetBits", nil)
	ser.RegisterConcrete(&ProposalHeartbeatMessage{}, "consensus/ProposalHeartbeat", nil)
}

// decodeMsg decodes the given bytes into a ConsensusMessage.
