...
public:
	...
	void compensateAward(const tc::Address &coinbase);
...
void Foundation::compensateAward(const tc::Address &coinbase){
	tc::BInt amount = tc::App::getInstance()->value();
	TC_RequireWithMsg(amount > BInt("0"), "transfer amount shoule not be zero");
	TC_Prints("compensateAward:");
	TC_Prints(coinbase.toString());
	TC_Prints(amount.toString());
	...
	TC_RequireWithMsg(candAddr.find(coinbase) != candAddr.end(), "address not in candidateList");
...
const std::string Foundation::GetVersion(){
	TC_Payable(false);
	return "20200107";
}