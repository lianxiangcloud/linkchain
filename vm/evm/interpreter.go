...
func NewInterpreter(evm *EVM, cfg Config) *Interpreter {
	...
	return &Interpreter{
		evm:      evm,
		cfg:      cfg,
		gasTable: config.Gastable(evm.BlockNumber),
		intPool:  newIntPool(),
	}
}
...
func (in *Interpreter) enforceRestrictions(op OpCode, operation operation, stack *Stack) error {
	...
	//instead of CALL if the EVM version is Byzantium or later.
	//This disallows state changes on the EVM level.
	if in.readOnly {
...
// ExecutionReverted which means revert-and-keep-gas-left.
func (in *Interpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	for atomic.LoadInt32(&in.evm.abort) == 0 {
		...
		case operation.reverts:
			return res, types.ExecutionReverted
