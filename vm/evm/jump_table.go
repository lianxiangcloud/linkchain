...
type (
	...
	gasFunc             func(cfg.GasTable, *EVM, *Contract, *Stack, *Memory, uint64) (uint64, error) // last parameter is the requested memory size as a uint64
...
func newFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		JUMPDEST: {
			execute:       opJumpdest,
			gasCost:       constGasFunc(cfg.JumpdestGas),
			validateStack: makeStackFunc(0, 0),
			valid:         true,
		},
		PUSH1: {
		...
		BALANCETOKEN: {
			execute:       opBalanceToken,
			gasCost:       gasBalance,
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		TRANSFERTOKEN: {
			execute:       opTransferToken,
			gasCost:       gasTransferToken,
			validateStack: makeStackFunc(3, 0),
			valid:         true,
			writes:        true,
			returns:       true,
