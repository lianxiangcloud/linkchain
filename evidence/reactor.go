...
func (evR *EvidenceReactor) SetP2PManager(pm p2p.P2PManager) {
	evR.sw = pm
	//evR.evpool.SetLogger(l)
}
...
func (evR *EvidenceReactor) Receive(chID byte, src p2p.Peer, msgBytes []byte) {
	...
	if err != nil {
		...
		evR.sw.StopPeerForError(src, err)
		return
	}
...
func (evR *EvidenceReactor) broadcastEvidenceRoutine(peer p2p.Peer) {
	...
	for {
		...
		if msg != nil {
			success := peer.Send(EvidenceChannel, ser.MustEncodeToBytesWithType(msg))
			retry = !success
		}
...
func (evR EvidenceReactor) checkSendEvidenceMessage(peer p2p.Peer, ev types.Evidence) (msg EvidenceMessage, retry bool) {
	...
	maxAge := evR.evpool.Status().ConsensusParams.EvidenceParams.MaxAge
...
func RegisterEvidenceMessages() {
	ser.RegisterInterface((*EvidenceMessage)(nil), nil)
	ser.RegisterConcrete(&EvidenceListMessage{}, "evidence/EvidenceListMessage", nil)
}

// decodeMsg decodes a byte-array into a EvidenceMessage.
