...
func keyLookupFromHeightAndHash(height uint64, hash []byte) []byte {
	return _key("%s/%s/%X", baseKeyLookup, bE(height), hash)
}

func keyOutqueue(evidence types.Evidence, priority int64) []byte {
	return _key("%s/%s/%s/%X", baseKeyOutqueue, bE(uint64(priority)), bE(evidence.Height()), evidence.Hash())
}
...
func (store *EvidenceStore) GetEvidence(height uint64, hash []byte) *EvidenceInfo {
...
func (store *EvidenceStore) AddNewEvidence(evidence types.Evidence, priority int64) bool {
	...
	eiBytes := ser.MustEncodeToBytes(ei)
...
func (store *EvidenceStore) MarkEvidenceAsCommitted(evidence types.Evidence) {
	...
	store.db.SetSync(lookupKey, ser.MustEncodeToBytes(ei))
}
