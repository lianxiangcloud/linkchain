...
func TestHeartbeatString(t *testing.T) {
	...
	require.Equal(t, "Heartbeat{1:000000000000 11/02 (0) <nil>}", hb.String())

	var key crypto.PrivKeyEd25519
	sig, err := key.Sign([]byte("blockchain"))
	require.NoError(t, err)
	hb.Signature = sig
	require.Equal(t, "Heartbeat{1:000000000000 11/02 (0) /B756748C6D4E.../}", hb.String())
}
