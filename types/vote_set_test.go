...
func randVoteSet(height uint64, round int, type_ byte, numValidators int, votingPower int64) (*VoteSet, *ValidatorSet, []PrivValidator) {
...
func withBlockHash(vote *Vote, blockHash []byte) *Vote {
	vote = vote.Copy()
	vote.BlockID.Hash = cmn.BytesToHash(blockHash)
	return vote
}
...
func TestAddVote(t *testing.T) {
	...
	vote := &Vote{
		...
		ValidatorSize:    voteSet.Size(),
		Height:           height,
		Round:            round,
		Type:             VoteTypePrevote,
		Timestamp:        time.Now().UTC(),
		BlockID:          BlockID{cmn.EmptyHash, PartSetHeader{}},
	}
...
func Test2_3MajorityRedux(t *testing.T) {
	...
	blockHash := common.BytesToHash(crypto.CRandBytes(32))
	blockPartsTotal := 123
...
func TestConflicts(t *testing.T) {
	...
	voteSet.SetPeerMaj23("peerA", BlockID{cmn.BytesToHash(blockHash1), PartSetHeader{}})
	...
	voteSet.SetPeerMaj23("peerA", BlockID{cmn.BytesToHash(blockHash2), PartSetHeader{}})
	...
	voteSet.SetPeerMaj23("peerB", BlockID{cmn.BytesToHash(blockHash1), PartSetHeader{}})
	...
	if !bytes.Equal(blockIDMaj23.Hash.Bytes(), blockHash1) {
...
func TestMakeCommit(t *testing.T) {
	...
	voteProto := &Vote{
		...
		BlockID:          BlockID{cmn.BytesToHash(blockHash), blockPartsHeader},
	}
