...
func BenchmarkValidatorSetCopy(b *testing.B) {
	...
	for i := 0; i < 1000; i++ {
		...
		val := NewValidator(pubKey, common.EmptyAddress, 0)
		if !vset.Add(val) {
...
func randValidator_() *Validator {
	val := NewValidator(randPubKey(), common.EmptyAddress, cmn.RandInt64())
...
func (valSet *ValidatorSet) toBytes() []byte {
	bz, err := ser.EncodeToBytes(valSet)
	if err != nil {
		panic(err)
	}
	return bz
...
func TestValidatorSetVerifyCommit(t *testing.T) {
	privKey := crypto.GenPrivKeyEd25519()
	pubKey := privKey.PubKey()
	v1 := NewValidator(pubKey, common.EmptyAddress, 1000)
	vset := NewValidatorSet([]*Validator{v1})

	chainID := "mychainID"
	blockID := BlockID{Hash: common.BytesToHash([]byte("hello"))}
	height := uint64(5)
	vote := &Vote{
	...
	badBlockID := BlockID{Hash: common.BytesToHash([]byte("goodbye"))}
	badHeight := height + 1
	badCommit := &Commit{
		BlockID:    blockID,
		Precommits: []*Vote{nil},
