...
func (voteSet *VoteSet) addVote(vote *Vote) (added bool, err error) {
	...
	if vote.ValidatorSize != voteSet.valSet.Size() {
		return false, errors.Wrapf(ErrVoteInvalidValidatorSize,
			"diff validator siez,local: %d vote: %d", voteSet.valSet.Size(), vote.ValidatorSize)
	}
...
func (voteSet *VoteSet) MarshalJSON() ([]byte, error) {
	voteSet.mtx.Lock()
	defer voteSet.mtx.Unlock()
	return ser.MarshalJSON(VoteSetJSON{
		voteSet.voteStrings(),
