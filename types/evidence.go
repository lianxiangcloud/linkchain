...
func RegisterEvidences() {
	ser.RegisterInterface((*Evidence)(nil), nil)
	ser.RegisterConcrete(&DuplicateVoteEvidence{}, "DuplicateVoteEvidence", nil)
	ser.RegisterConcrete(&FaultValidatorsEvidence{}, "FaultValidatorsEvidence", nil)

	// mocks
	ser.RegisterConcrete(MockGoodEvidence{}, "MockGoodEvidence", nil)
	ser.RegisterConcrete(MockBadEvidence{}, "MockBadEvidence", nil)
}
...
func (fve *FaultValidatorsEvidence) Equal(ev Evidence) bool {
	if _, ok := ev.(*FaultValidatorsEvidence); !ok {
		return false
	}
	if fve.Height() != ev.Height() {
		return false
	}
	// just check their hashes
	evHash := aminoHasher(ev).Hash()
	return bytes.Equal(fve.Hash(), evHash)
}

func (fve *FaultValidatorsEvidence) String() string {
	if fve.FaultVal == nil {
		return fmt.Sprintf("height:%d,Faultval:nil Proposer:%s ProduceRound:%d",
			fve.Height(), fve.Proposer.Address().String(), fve.Round)
	}
	return fmt.Sprintf("height:%d,Faultval:%s Proposer:%s ProduceRound:%d",
		fve.Height(), fve.FaultVal.Address().String(), fve.Proposer.Address().String(), fve.Round)
}
